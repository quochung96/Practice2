package com.likelion.Practice.Week8.Aspect;
...
public class ServiceAspect {
    ...
    @Before("execution(* com.example.Practice.Week8.service.*.*(..))")
    public void before(JoinPoint joinPoint){
        LOGGER.info("before called "+ joinPoint.toString());
    }
    @After("execution(* com.example.Practice.Week8.service.*.*(..))")
    public void after(JoinPoint joinPoint){
        LOGGER.info("after called "+ joinPoint.toString());
    }
    @AfterThrowing("execution(* com.example.Practice.Week8.service.*.*(..))")
